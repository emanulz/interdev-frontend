...
export default class DataTable extends React.Component {
  ...
  componentDidUpdate() {
    ...
    $(this.refs.table).DataTable({
      destroy: true,
      iDisplayLength: 50,
      dom: "<'row'<'col-xs-5'l><'col-xs-7'f>>" +
           "<'row topPagination'<'col-sm-12'p>>" +
           "<'row'<'col-sm-12'tr>>" +
           "<'row bottomPagination'<'col-sm-3'i><'col-sm-9'p>>",
      lengthMenu: [
        [
          10, 25, 50, 100, -1
        ],
      ...
      return <tr key={el[idField]}>
        {headerOrder.map(header => {
          ...
          switch (header.type) {
            case 'price':
            {
              item = <td key={`${el[idField]}_${header.field}`} data-order={itemToRender}>
            ...
            case 'bool':
            {
              ...
              item = <td style={{
                textAlign: 'center'
              }} key={`${el[idField]}_${header.field}`} data-order={itemToRender}>
                <span className={icon} />
              </td>
              break
            }
