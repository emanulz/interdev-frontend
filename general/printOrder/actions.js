...
export function loadOrderToPrint(consecutive) {
  const url = `/api/orders/?consecutive=${consecutive}`

  return function(dispatch) {
    // GET THE SALE AND DISPATCH
    axios.get(url).then(function(response) {
      // FIRST CLEAR REDUCER DATA
      dispatch({type: 'CLEAR_PRINT_ORDER', payload: ''})

      if (response.data.count > 0) {
        // IF THERE IS MORE THAN ONE ELEMENT FILTERED
        if (response.data.count > 1) {
          alertify.alert('ATENCIÓN', `Existe mas de una orden de compra con el consecutivo:
        ...
        dispatch({type: 'SET_PRINT_ORDER', payload: response.data.results[0]})
        // THEN IF THERE IS A ORDER, FETCH IT AND DISPATCH
        dispatch({type: 'FETCHING_DONE', payload: ''})
        dispatch({type: 'SHOW_PRINT_ORDER_PANEL', payload: ''})
